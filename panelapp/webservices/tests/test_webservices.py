...
class TestWebservices(TransactionTestCase):
    def setUp(self):
        ...
        self.region2 = RegionFactory(
            panel__panel__status=GenePanel.STATUS.public, position_37=None
        )
    ...
    def test_internal_panel(self):
        ...
        self.assertEqual(r.status_code, 200)
        self.assertEqual(
            r.json(), ["Query Error: {} not found.".format(self.gps.panel.pk)]
        )
    ...
    def test_get_panel_name(self):
        r = self.client.get(
            reverse_lazy("webservices:get_panel", args=(self.gpes.panel.panel.name,))
        )
        self.assertEqual(r.status_code, 200)
    ...
    def test_get_panel_name_deleted(self):
        r = self.client.get(
            reverse_lazy(
                "webservices:get_panel", args=(self.gpes_deleted.panel.panel.name,)
            )
        )
        self.assertEqual(r.status_code, 200)
        self.assertEqual(
            r.json(),
            ["Query Error: {} not found.".format(self.gpes_deleted.panel.panel.name)],
        )

    def test_get_panel_internal(self):
        r = self.client.get(
            reverse_lazy(
                "webservices:get_panel", args=(self.gpes_internal.panel.panel.name,)
            )
        )
        self.assertEqual(r.status_code, 200)
        self.assertEqual(
            r.json(),
            ["Query Error: {} not found.".format(self.gpes_internal.panel.panel.name)],
        )

    def test_get_panel_retired(self):
        r = self.client.get(
            reverse_lazy(
                "webservices:get_panel", args=(self.gpes_retired.panel.panel.name,)
            )
        )
        self.assertEqual(r.status_code, 200)
        self.assertEqual(
            r.json(),
            ["Query Error: {} not found.".format(self.gpes_retired.panel.panel.name)],
        )

    def test_get_panel_pk(self):
        r = self.client.get(
            reverse_lazy("webservices:get_panel", args=(self.gpes.panel.panel.pk,))
        )
        self.assertEqual(r.status_code, 200)
    ...
    def test_get_search_gene(self):
        url = reverse_lazy(
            "webservices:search_genes", args=(self.gpes.gene_core.gene_symbol,)
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
    ...
    def test_search_by_gene(self):
        ...
        r = self.client.get(
            "{}?ModeOfPathogenicity={}".format(url, self.gpes.mode_of_pathogenicity)
        )
        self.assertEqual(r.status_code, 200)

        r = self.client.get(
            "{}?LevelOfConfidence={}".format(url, self.gpes.saved_gel_status)
        )
        self.assertEqual(r.status_code, 200)
        ...
        r = self.client.get(
            "{}?Evidences={}".format(url, self.gpes.evidence.first().name)
        )
        self.assertEqual(r.status_code, 200)
        ...
        multi_genes_url = reverse_lazy(
            "webservices:search_genes", args=(multi_genes_arg,)
        )
    ...
    def test_strs_in_panel(self):
        r = self.client.get(
            reverse_lazy("webservices:get_panel", args=(self.str.panel.panel.pk,))
        )
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["result"]["STRs"][0]["Name"], self.str.name)
        self.assertEqual(
            r.json()["result"]["STRs"][0]["GRCh37Coordinates"],
            [self.str.position_37.lower, self.str.position_37.upper],
        )
        self.assertEqual(
            r.json()["result"]["STRs"][0]["GRCh38Coordinates"],
            [self.str.position_38.lower, self.str.position_38.upper],
        )
        self.assertEqual(
            r.json()["result"]["STRs"][0]["PathogenicRepeats"],
    ...
    def test_super_panel(self):
        ...
        r_direct = self.client.get(
            reverse_lazy("webservices:get_panel", args=(self.gps_public.panel.pk,))
        )
        result_genes = list(
            sorted(
                r_direct.json()["result"]["Genes"], key=lambda x: x.get("GeneSymbol")
            )
        )

        r = self.client.get(
            reverse_lazy("webservices:get_panel", args=(super_panel.panel.pk,))
        )
        self.assertEqual(r.status_code, 200)
        self.assertEqual(
            result_genes,
            list(
                sorted(r.json()["result"]["Genes"], key=lambda x: x.get("GeneSymbol"))
            ),
        )
    ...
    def test_no_loss_of_function(self):
        ...
        r = self.client.get(
            "{}?ModeOfPathogenicity={}".format(url, "no_loss_of_function")
        )
        ...
        self.assertEqual(
            r.json()["results"][0]["GeneSymbol"], self.gpes.gene.get("gene_symbol")
        )

    def test_region_in_panel(self):
        r = self.client.get(
            reverse_lazy("webservices:get_panel", args=(self.region.panel.panel.pk,))
        )
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["result"]["Regions"][0]["Name"], self.region.name)
        self.assertEqual(
            r.json()["result"]["Regions"][0]["GRCh37Coordinates"],
            [self.region.position_37.lower, self.region.position_37.upper],
        )
        self.assertEqual(
            r.json()["result"]["Regions"][0]["GRCh38Coordinates"],
            [self.region.position_38.lower, self.region.position_38.upper],
        )
        self.assertEqual(
            r.json()["result"]["Regions"][0]["TriplosensitivityScore"],
            self.region.triplosensitivity_score,
        )
        self.assertEqual(
            r.json()["result"]["Regions"][0]["TypeOfVariants"],
            self.region.type_of_variants,
        )

    def test_region_in_panel_empty_pos37(self):
        r = self.client.get(
            reverse_lazy("webservices:get_panel", args=(self.region2.panel.panel.pk,))
        )
        self.assertEqual(r.status_code, 200)
        ...
        self.assertEqual(
            r.json()["result"]["Regions"][0]["TriplosensitivityScore"],
            self.region2.triplosensitivity_score,
        )
        self.assertEqual(
            r.json()["result"]["Regions"][0]["TypeOfVariants"],
            self.region2.type_of_variants,
        )

    def test_region_filters(self):
        url = reverse_lazy("webservices:get_panel", args=(self.region.panel.panel.pk,))
        r = self.client.get(
            "{}?HaploinsufficiencyScore={}".format(
                url, self.region.haploinsufficiency_score
            )
        ).json()
        self.assertEqual(
            r["result"]["Regions"][0]["HaploinsufficiencyScore"],
            self.region.haploinsufficiency_score,
        )

        r = self.client.get(
            "{}?TriplosensitivityScore={}".format(
                url, self.region.triplosensitivity_score
            )
        ).json()
        self.assertEqual(
            r["result"]["Regions"][0]["TriplosensitivityScore"],
            self.region.triplosensitivity_score,
        )
    ...
    def test_list_entities(self):
        ...
        self.assertEqual(
            len([r for r in res["results"] if r["EntityType"] == "str"]), 1
        )
        self.assertEqual(
            len([r for r in res["results"] if r["EntityType"] == "region"]), 2
        )
