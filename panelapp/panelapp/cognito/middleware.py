...
logger = logging.getLogger("panelapp.cognito.middleware")
...
class ALBAuthMiddleware:
    ...
    def process_login(self, request):
        ...
        :return:
        """
        logger.debug("Cognito process login")

        meta = request.META
        if not self.has_amzn_oidc_headers(meta):
            return

        encoded_jwt_data = meta.get(settings.AWS_AMZN_OIDC_DATA)
    ...
    def login_cognito_user(self, request, email, first_name, last_name):
        ...
        if user:
            ...
            logger.info("Cognito user {} has login".format(user.id))
    ...
    def verify_amzn_jwt_structure(encoded_jwt_data):
        ...
        :return:
        """
        return len(encoded_jwt_data.split(".")) == settings.AWS_JWT_SECTIONS
    ...
    def get_public_key(self, encoded_jwt_data):
        ...
        resp = requests.get(settings.AWS_ELB_PUBLIC_KEY_ENDPOINT + kid)
    ...
    def get_alb_session_cookies(http_cookie):
        ...
        ks = []
        for k in cookie:
            if k.startswith(settings.AWS_ELB_SESSION_COOKIE_PREFIX):
                ks.append(k)
        return ks

    def logout_cognito_user(self, request, response):
        """
        When application needs to log out an authenticated user, it should set the expiration time of the
        ...
        if alb_session_cookies:
            resp = HttpResponseRedirect(settings.AWS_COGNITO_IDP_LOGOUT_ENDPOINT)
