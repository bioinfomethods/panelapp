...
class GenePanelSpanshotView(EntityMixin, DetailView):
    ...
    def get_context_data_gene(self, ctx):
        ...
        ctx["next_gene"] = (
            None if cgi == len(ctx["panel_genes"]) - 1 else ctx["panel_genes"][cgi + 1]
        )
    ...
    def get_context_data_str(self, ctx):
        ...
        ctx["next_str"] = (
            None if cgi == len(ctx["panel_strs"]) - 1 else ctx["panel_strs"][cgi + 1]
        )
    ...
    def get_context_data_region(self, ctx):
        ...
        ctx["next_region"] = (
            None
            if cgi == len(ctx["panel_regions"]) - 1
            else ctx["panel_regions"][cgi + 1]
        )
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if self.is_gene() or (self.object.gene and self.object.gene.get("gene_symbol")):
            ctx["sharing_panels"] = GenePanelSnapshot.objects.get_shared_panels(
                self.object.gene.get("gene_symbol"), all=is_admin, internal=is_admin
            )
        else:
            ctx["sharing_panels"] = []

        ctx["panel_genes"] = list(
            self.panel.get_all_genes_extra.values(
                "pk", "gene", "evaluators", "number_of_evaluations", "saved_gel_status"
            )
        )
        ctx["panel_strs"] = list(
            self.panel.get_all_strs_extra.values(
                "pk",
                "gene",
                "name",
                "evaluators",
        ...
        ctx["panel_regions"] = list(
            self.panel.get_all_regions_extra.values(
                "pk",
                "gene",
                "name",
                "verbose_name",
