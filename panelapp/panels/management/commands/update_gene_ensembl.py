...
def get_active_panels():
    return (
        GenePanelSnapshot.objects.get_active(all=True, internal=True, superpanels=False)
...
def process(json_data):
    ...
    with transaction.atomic():
        ...
        strs_in_panels = STR.objects.get_active(pks=active_panels).filter(
            gene__gene_symbol__in=gene_keys
        )
        grouped_strs = {
        ...
        regions_in_panels = Region.objects.get_active(pks=active_panels).filter(
            gene__gene_symbol__in=gene_keys
        )
        grouped_regions = {
        ...
        for gene_symbol in gene_keys:
            try:
                ...
                click.secho(
                    "Skipping {}. This gene is missing from the db".format(gene_symbol),
                    fg="red",
                )
                continue
