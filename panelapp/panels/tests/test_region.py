...
class RegionTest(LoginGELUser):
    def test_region_data_copied(self):
        ...
        region = active_panel.add_region(
            self.gel_user,
            "ABC",
            {
                "gene": gpes.gene_core,
    ...
    def test_add_region_to_panel(self):
        ...
        new_region = GenePanel.objects.get(pk=gps.panel.pk).active_panel.get_region(
            region_data["name"]
        )
        assert sorted(
    ...
    def test_edit_region_in_panel(self):
        ...
        self.assertEqual(
            int(region_data["position_37_1"]), new_region.position_37.upper
        )
    ...
    def test_remove_sources(self):
        ...
        region = RegionFactory(penetrance=GenePanelEntrySnapshot.PENETRANCE.Incomplete)
        url = reverse_lazy(
            "panels:edit_entity",
            kwargs={
                "pk": region.panel.panel.pk,
        ...
        region_data = {
            ...
            "publications": ";".join(
                [publication for publication in region.publications]
            ),
    ...
    def test_edit_region_name_ajax(self):
        ...
        old_gps = HistoricalSnapshot.objects.get(panel=region.panel.panel)

        # check panel has no previous gene
        assert new_gps.has_region(old_region_name) is False

        # test previous panel contains old gene
        assert old_region_name in [g["entity_name"] for g in old_gps.data["regions"]]
