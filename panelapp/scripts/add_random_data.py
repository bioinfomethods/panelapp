...
def get_random_active_panel():
    return (
        GenePanelSnapshot.objects.get_active(all=True, internal=True, superpanel=False)
        .order_by("?")
        .first()
    )
...
def random_gene_data(gene):
    return {
        ...
        "rating": random.choice(
            [Evaluation.RATINGS.AMBER, Evaluation.RATINGS.GREEN, Evaluation.RATINGS.RED]
        ),
...
def random_region_data(region):
    return {
        ...
        "position_37": NumericRange(
            random.randint(1, 20000), random.randint(20001, 40000)
        ),
...
def random_str_data(data_str):
    return {
        ...
        "repeated_sequence": "".join(
            random.choice(["A", "T", "C", "G"]) for _ in range(random.randint(5, 20))
        ),
...
def add_some_entropy(gps):
    ...
    user = (
        Reviewer.objects.filter(user_type=Reviewer.TYPES.GEL).order_by("?").first().user
    )
...
def process(iterations=100):
    with transaction.atomic():
        ...
        for _ in range(iterations):
            gps = (
                GenePanelSnapshot.objects.get_active(
